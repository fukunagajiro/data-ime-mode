...
> **実行時の訂正（2026-07-26）**: 下の当初案は、狙った回帰（`ping` が `lastChange` を
> 記録してしまうバグ）を検出できない。`$baseline` の取得からポート断まで IME の状態が
> 変わらないため、`finally` の復帰が**同値の書き戻し**になり、アサーションが通ってしまう。
> 実際の危険（prewarm の ping の後、ポートが切れるまでの間にユーザーが自分で IME を
> 切り替える）を再現すること。順序は ping → `Set-State` で状態を変える → 変えた値を
> `$afterPing` に捕まえる → ポート断 → **`$afterPing` と一致すること**を確認する、とする。
> 「変えられていること」自体を確かめる前提アサーションも 1 つ置く。

以下は当初の記述（アサーションの比較対象だけが上記のとおり変わる）。
